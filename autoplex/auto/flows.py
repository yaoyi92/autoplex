...
class PhononDFTMLDataGenerationFlow(Maker):
    ...
    n_struc: int = 1
    displacements: list[float] = field(default_factory=lambda: [0.1])  #TODO: wanna make sure that 0.1 is always included, no matter what the user does
    min_length: int = 20
    symprec: float = 0.01
    sc: bool = False
    ...
    def make(
        ...
        for struc_i, structure in enumerate(structure_list):
            ...
            autoplex_datagen = PhononDFTMLDataGenerationFlow(name="test",
                                                             ...
                                                             sc=self.sc).make(structure=structure, mpid=mpid)
        ...
        autoplex_ml_phonon = PhononMLCalculationJob(structure=benchmark_structure, min_length=self.min_length,
        ...
        if mpbm not in mpids:
            DFTphonons = DFTPhononMaker(symprec = self.symprec,
                                        ...
                                        min_length = self.min_length).make(structure = benchmark_structure)
            DFTphonons = update_user_incar_settings(DFTphonons, {"NPAR": 4})
            flows.append(DFTphonons)

            dft_reference = DFTphonons.output
        else:
            dft_reference = datagen[mpbm]["phonon_data"][0]  # [0] because we only need the first entry for the displacement = 0.1


        autoplex_bm = PhononDFTMLBenchmarkFlow(name = "testBM").make(structure=benchmark_structure, mpid = mpbm,
                                                                     ml_reference=autoplex_ml_phonon.output,
                                                                     dft_reference=dft_reference)
        flows.append(autoplex_bm)
        collect.append(autoplex_bm.output)

        collect_bm = CollectBenchmark(benchmark_structure=benchmark_structure, mpbm=mpbm, rms=collect,
