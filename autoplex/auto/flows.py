...
class PhononDFTMLDataGenerationFlow(Maker):
    ...
    def make(self, structure: Structure, mpid):
        ...
        for displacement in self.displacements:
            ...
            DFTphonons = update_user_incar_settings(DFTphonons, {"NPAR": 4})
            flows.append(DFTphonons)
            DFTphonons_output.append(
                DFTphonons.output
            )  # I have no better solution to this now
            DFTphonons_dir_output.append(
                DFTphonons.output.jobdirs.displacements_job_dirs
            )
...
class PhononDFTMLFitFlow(Maker):
    ...
    def make(self, species, isolated_atoms, fitinput, **fit_kwargs):
        """
        ...
        MLfit = MLIPFitMaker(name="GAP").make(
            species_list=species,
            iso_atom_energy=isolated_atoms,
            fitinput=fitinput,
            **fit_kwargs,
        )
        flows.append(MLfit)
