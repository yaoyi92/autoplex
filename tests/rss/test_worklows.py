...
from autoplex.data.common.jobs import Data_preprocessing, Sampling, VASP_collect_data
...
@job
def mock_do_RSS_iterations(
    input: dict[str, Optional[Any]] = {
        "test_error": None,
    ...
    **fit_kwargs,
    ...
    if (
        input["test_error"] is not None
        and input["test_error"] > stop_criterion
        and input["current_iter"] < max_iteration_number
    ):
        if input["kt"] > 0.15:
            kt = input["kt"] - 0.1
        else:
